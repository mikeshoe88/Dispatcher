...
app.action('complete_task', async ({ body, ack, client }) => {
  ...
  try {
    ...
    await fetch(pdUrl, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ done: true, marked_as_done_time: new Date().toISOString() }) });
...
function verify(raw, sig) { try { return crypto.timingSafeEqual(Buffer.from(sign(raw)), Buffer.from(String(sig))); } catch { return false; } }
...
async function buildWorkOrderPdfBuffer({ activity, dealTitle, typeOfService, location }) {
  const completeUrl = makeSignedCompleteUrl({ aid: String(activity.id), did: activity.deal_id ? String(activity.deal_id) : '', cid: SCHEDULE_CHANNEL });
...
async function uploadPdfToSlack({ channel, filename, pdfBuffer, title, initialComment = '' }) {
  // files.upload is deprecated; use files.uploadV2
...
async function uploadPdfToPipedrive({ dealId, pdfBuffer, filename }) {
  ...
  const resp = await fetch(`https://api.pipedrive.com/v1/files?api_token=${PIPEDRIVE_API_TOKEN}`, { method: 'POST', body: form });
  const j = await resp.json();
  if (!j?.success) { console.error('Pipedrive file upload failed:', j); throw new Error('PD file upload failed'); }
  return j;
}
...
expressApp.post('/pipedrive-task', async (req, res) => {
  try {
    ...
    const aRes = await fetch(`https://api.pipedrive.com/v1/activities/${activity.id}?api_token=${PIPEDRIVE_API_TOKEN}`);
    ...
    if (aJson?.success && aJson.data) {
      ...
      if (dealId && dealId !== 'N/A') {
        const dRes = await fetch(`https://api.pipedrive.com/v1/deals/${dealId}?api_token=${PIPEDRIVE_API_TOKEN}`);
    ...
    const message = {
      ...
      blocks: [
        { type: 'section', text: { type: 'mrkdwn', text: `📌 *New Task*\n• *${activity.subject}*\n🗕️ Due: ${activity.due_date || 'No due date'}\n📜 Note: ${fullNote}\n🏷️ Deal ID: ${dealId} - *${dealTitle}*\n📦 Type of Service: ${typeOfService}\n📍 Location: ${location}` } },
        { type: 'actions', elements: [ { type: 'checkboxes', action_id: 'complete_task', options: [ { text: { type: 'mrkdwn', text: 'Mark as complete' }, value: `task_${activity.id}` } ] } ] }
      ]
    };
    ...
    try {
      const pdfBuffer = await buildWorkOrderPdfBuffer({ activity, dealTitle, typeOfService, location });
      const safe = (s) => (s || '').toString().replace(/[^\w\-]+/g, '_').slice(0, 60);
      const filename = `WO_${safe(dealTitle)}_${safe(activity.subject)}.pdf`;

      await uploadPdfToSlack({ channel: SCHEDULE_CHANNEL, filename, pdfBuffer, title: `Work Order — ${activity.subject || ''}`, initialComment: '📄 Work Order PDF (scan QR to complete)' });
      await uploadPdfToPipedrive({ dealId, pdfBuffer, filename });

      // Optional PD note with the QR-complete link
      const completeUrl = makeSignedCompleteUrl({ aid: String(activity.id), did: dealId !== 'N/A' ? String(dealId) : '', cid: SCHEDULE_CHANNEL });
      await fetch(`https://api.pipedrive.com/v1/notes?api_token=${PIPEDRIVE_API_TOKEN}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ deal_id: dealId, content: `Work Order PDF attached.\nScan to complete: ${completeUrl}` }) });
    } catch (e) { console.error('PDF/QR upload failed:', e); }
...
expressApp.get('/wo/complete', async (req, res) => {
  try {
    ...
    if (channel) {
      const text = ok ? `✅ Task *${aid}* marked complete in Pipedrive${did ? ` (deal ${did})` : ''}.` : `⚠️ Tried to complete task *${aid}* but Pipedrive didn’t confirm success.`;
